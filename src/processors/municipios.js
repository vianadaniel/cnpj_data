 ...
 */
async function processMunicipiosFile(filePath) {
    ...
    try {
        ...
        await client.query(`
            CREATE TEMP TABLE temp_municipios (
                codigo TEXT,
                nome TEXT,
                uf TEXT
            ) ON COMMIT DROP
        ...
        const stream = fs.createReadStream(filePath, { encoding: 'latin1' })
            .pipe(csv({
                separator: ';',
                headers: ['codigo', 'nome', 'uf'],
                skipLines: 0
            }));

        // Processar cada linha do CSV
        for await (const row of stream) {
            if (row.codigo && row.nome && row.uf) {
                ...
                batch.push([row.codigo, row.nome, row.uf]);
                ...
                if (batch.length >= BATCH_SIZE) {
                    ...
                    for (const [codigo, nome, uf] of batch) {
                        values.push(codigo, nome, uf);
                        placeholders.push(`($${paramCount}, $${paramCount + 1}, $${paramCount + 2})`);
                        paramCount += 3;
                    }

                    await client.query(`
                        INSERT INTO temp_municipios (codigo, nome, uf)
                        VALUES ${placeholders.join(', ')}
                    `, values);

                    processed += batch.length;
                    batch = [];
                    console.log(`Processados ${processed} registros de municípios`);
                }
            }
        }
        ...
        const tempCount = await client.query('SELECT COUNT(*) FROM temp_municipios');
        ...
        const result = await client.query(`
            ...
            SELECT codigo, nome, uf FROM temp_municipios
        ...
        const finalCount = await client.query('SELECT COUNT(*) FROM municipios');
        console.log(`Total de registros na tabela final: ${finalCount.rows[0].count}`);

        await client.query('COMMIT');
        console.log(`Processamento de municípios concluído. Total processado: ${processed} registros`);

    } catch (error) {
        await client.query('ROLLBACK');
        console.error('Erro ao processar arquivo de municípios:', error);
        throw error;
    } finally {
        client.release();
    }
