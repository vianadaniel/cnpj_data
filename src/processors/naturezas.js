...
/**
 * Processa um arquivo de naturezas jurídicas e insere os dados no banco de dados
 * @param {string} filePath - Caminho do arquivo a ser processado
 */
async function processNaturezasFile(filePath) {
    console.log(`Iniciando processamento de naturezas jurídicas: ${filePath}`);
    ...
    try {
        ...
        await client.query(`
            CREATE TEMP TABLE temp_naturezas (
                codigo TEXT,
                descricao TEXT
            ) ON COMMIT DROP
        `);
        ...
        for await (const row of stream) {
            if (row.codigo && row.descricao) {
                ...
                if (batch.length >= BATCH_SIZE) {
                    ...
                    await client.query(`
                        INSERT INTO temp_naturezas (codigo, descricao)
                        VALUES ${placeholders.join(', ')}
                    `, values);

                    processed += batch.length;
                    batch = [];
                    console.log(`Processados ${processed} registros de naturezas jurídicas`);
                }
            }
        }
        ...
        const tempCount = await client.query('SELECT COUNT(*) FROM temp_naturezas');
        ...
        const result = await client.query(`
            ...
            SELECT codigo, descricao FROM temp_naturezas
        ...
        const finalCount = await client.query('SELECT COUNT(*) FROM naturezas_juridicas');
        console.log(`Total de registros na tabela final: ${finalCount.rows[0].count}`);

        await client.query('COMMIT');
        console.log(`Processamento de naturezas jurídicas concluído. Total processado: ${processed} registros`);

    } catch (error) {
        await client.query('ROLLBACK');
        console.error('Erro ao processar arquivo de naturezas jurídicas:', error);
        throw error;
    } finally {
        client.release();
    }
